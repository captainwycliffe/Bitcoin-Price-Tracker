...
export default function ResultRow({
  ...
  return (
    ...
    href={url}
    target='_blank'
     className="block relative border min-h-[64px] border-white/10 rounded-lg bg-gradient-to-r from-purple-500/10 to-blue-500/10 p-4 my-4 overflow-hidden">
      <div className="flex gap-4">
        {providerName && (
          <div className="grow items-centre flex">
          <img
            src={logos[providerName].source}
            alt=""
            className={"h-6 " + (logos[providerName]?.invert ? 'invert' :  '' )}
          />
          </div>
        )}
        { btc && (
        ...
        { new Intl.NumberFormat('sw-KE', {minimumFractionDigits:8}).format(parseFloat (btc))}
        </span>
      ...
      {loading && (
        <div className="inset-0 absolute bg-gradient-to-r from-transparent via-blue-900 to-transparent skeleton-animation border-t border-white/25"/>
      )}
    </a>
  );
}
